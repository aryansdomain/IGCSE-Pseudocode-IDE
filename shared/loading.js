...
style.textContent = `
    ...
    #appLoader .dot { width:10px; height:10px; border-radius:50%; animation:boot-b 1s infinite alternate; background: var(--text, #fff); }
    #appLoader .dot:nth-child(2) { animation-delay:.15s }
    #appLoader .dot:nth-child(3) { animation-delay:.3s }
    #appLoader .loading-text { font-family:system-ui,-apple-system,sans-serif; font-size:14px; color: var(--text, #fff); font-weight:500; transform:translateX(3px); }
    #appLoader .error-message { font-family:system-ui,-apple-system,sans-serif; font-size:12px; color: var(--text, #fff); opacity:0.7; font-weight:400; margin-top:4px; text-align:center; }
    @keyframes boot-b{ from {transform:scale(.6); opacity:.5 } to { transform:scale(1); opacity:1} }
`;
document.head.appendChild(style);

function showLoadingDots() {
    if (shown || appReady) return;
    shown = true;
...
function clear() {
    if (shown || appReady) clearTimeout(timer);
    loadingEl = null;
...
function onError() {
    if (!appReady && loadingEl) showReloadPrompt();
}
