export function initRunCtrl({
    ...
    let code_ran_method = 'button';
    let code_ran_runtime = 0; let startTime = 0;
    let code_ran_size = 0;
    let code_ran_success = false;
    ...
    function finishRun(localRunId) {
        ...
        // record analytics
        try {
            window.code_ran && window.code_ran({
                code_ran_method,
                code_ran_runtime,
                code_ran_size,
                code_ran_success
    ...
    function attachWorkerHandlers(localRunId) {
        worker.onmessage = async (e) => {
            ...
            // stops the program
                ...
                // set analytics vars
                code_ran_runtime = performance.now() - startTime;
                code_ran_success = (type === 'done'); // mark as fail if error
    ...
    function run(method = 'button') {
        ...
        // set analytics vars
        code_ran_method = method;
        startTime = performance.now(); code_ran_runtime = 0;
        code_ran_size = getCode().length;
        code_ran_success = false;
        alreadyOutput = false;
        ...
        loadingTimer = setTimeout(() => {
            if (isRunning && newRunId === runId) onLoadingChange(true);
        }, 100);
    }
    function stop() {
    ...
    function provideInput(line) {
        ...
        loadingTimer = setTimeout(() => {
            if (isRunning) onLoadingChange(true);
        }, 100);
    }
