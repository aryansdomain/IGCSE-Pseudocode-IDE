...
import { runGraph }  from '../../flowcharts/runtime/graph.js';

let currentResolve = null;
self.readInput = (name, index) => {
    // await a Promise - set the resolve function (finishes the Promise) to currentResolve
    self.postMessage({ type: 'input', name, index });                               //
...
self.onmessage = async (e) => {                                          //
    ...
    // run
    if (msg.type === 'run') {
        self.flowchart = !!msg.flowchart; // is a flowchart
        try {
            if (msg.graph) await runGraph(msg.graph, msg.arraySpecs); // if a flowchart
            else           await interpret(msg.code);
            ...
            if (e.formatted) {
                self.postMessage({
                    ...
                    errorType: e.type,
                    code:      e.code,
                    nodeId:    e.nodeId // flowchart node to highlight
                });
            } else { // JS error
                self.postMessage({
                    type: 'error',
