...
import { consts, globalScope, addProperties,
         setDeclared, isDeclared, setInitialized,
    ...
    for (let i = 0; i < code.length; i++) {
        ...
        if (r = line.match(/^(PROCEDURE|FUNCTION)\s+([A-Za-z][A-Za-z0-9_]*)\b/i)) {
            ...
            if (r[1].toUpperCase() === 'PROCEDURE') {
                const {nextLineNum} = readLines(code, i + 1, 'PROCEDURE',    /^PROCEDURE\b/i,
                                                             'ENDPROCEDURE', /^(ENDPROCEDURE)\b/i,
                                                             lineWithComments);
                i = nextLineNum;
            } else {
                const {nextLineNum} = readLines(code, i + 1, 'FUNCTION',    /^FUNCTION\b/i,
                                                             'ENDFUNCTION', /^(ENDFUNCTION)\b/i,
                                                             lineWithComments);
                i = nextLineNum;
            }
            continue;
        ...
        if (r = line.match(/^INPUT\s+(.+)$/i)) {
            ...
            const inputText = await self.readInput(obj.name, obj.index); // index for array elements

            // assign
            assign(scope, obj, inputText, varText, true, lineWithComments);
            if (!parsed.hasIndices && !getDeclaredType(scope, obj.name)) {
                // if var is assigned a number, set type to be a REAL and set assumed if assigned an integer
                const inputVal = obj.read();
                const type = getValType(inputVal, inputText);
                setType(scope, obj.name, typeof inputVal === 'number' ? 'REAL' : type, type === 'INTEGER');
            }
            setInitialized(scope, obj.name);
            if (self.flowchart) self.postMessage({ type: 'variable', name: obj.name, index: obj.index, value: obj.read() });
        ...
        if (r = line.match(/^READFILE\s+(.+?)\s*,\s*(.+)$/i)) {
            ...
            if (!parsed.hasIndices && !getDeclaredType(scope, obj.name)) {
                // if var is assigned a number, set type to be a REAL and set assumed if assigned an integer
                const fileVal = obj.read();
                const type = getValType(fileVal, fileLine);
                setType(scope, obj.name, typeof fileVal === 'number' ? 'REAL' : type, type === 'INTEGER');
            }
            setInitialized(scope, obj.name);
            continue;
        }
        ...
        if (r = line.match(/^FOR\s+([A-Za-z][A-Za-z0-9_]*)\s*(?:←|<-|<--)\s*(.+?)\s+TO\s*(.+?)(?:\s+STEP\s+(.+))?\s*$/i)) {
            ...
            assertAssignCompatible(getDeclaredType(scope, name), 1, name, lineWithComments);
            ...
            for (dS[dN] = start;   (step > 0) ? dS[dN] <= end : dS[dN] >= end;   dS[dN] += step) {
                if (self.flowchart) { self.postMessage({ type: 'iteration' }); self.postMessage({ type: 'variable', name, value: dS[dN] }); }

                await runCode(scope, forBlock, allowReturn);
                count++;
        ...
        if (r = line.match(/^WHILE\s+(.+?)(?:\s+(DO))?\s*$/i)) {
            ...
            while (true) {
                ...
                if (self.flowchart) self.postMessage({ type: 'iteration' });

                await runCode(scope, whileBlock, allowReturn);
                count++;
        ...
        if (r = line.match(/^REPEAT$/i)) {
            ...
            do {
                ...
                await runCode(scope, repeatBlock, allowReturn);
                count++;
        ...
        if (r = line.match(/^(.+?)\s*(?:←|<--|<-)\s*(.+)$/)) {
            ...
            if (!isArray(scope, parsed.name) && !getDeclaredType(scope, obj.name)) { // assigned for the first time
                ...
                // if var is assigned a number, set type to be a REAL and set assumed if assigned an integer
                const type = getValType(val, valText);
                setType(scope, obj.name, valType || (typeof val === 'number' ? 'REAL' : type), !valType && type === 'INTEGER');
            }
            continue;
        }
        ...
        if (r = line.match(/^IF\s+(.+?)(?:\s+(THEN)(?:\s+(.+))?)?\s*$/i)) {
            ...
            // execute
            if (cond) await runCode(scope, thenBlock, allowReturn);
            else      await runCode(scope, elseBlock, allowReturn);
            continue;
        }
        ...
        if (r = line.match(/^CASE\s+OF\s+(.+)$/i)) {
            ...
            // run first matched case, or otherwise if no case matched
                 if (matched  ) await runCode(scope, matched.block,   allowReturn);
            else if (otherwise) await runCode(scope, otherwise.block, allowReturn);
            continue;
        }
